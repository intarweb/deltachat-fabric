...
# ---- OpConnectStore against a mocked op-connect ---------------------------------------------
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.method == "GET":
            return httpx.Response(200, json={"fields": [
                {"label": "alpha", "type": "CONCEALED", "value": "stored-pw-abc"}]})
        writes.append(request.method)
        return httpx.Response(200, json={})

    store = OpConnectStore(base_url="http://op.test", token="t", vault="v", item="i",
                           client=httpx.Client(transport=httpx.MockTransport(handler)))
    assert store.get_or_create("alpha") == "stored-pw-abc"
    assert writes == []                                                # existing → no mint/write


def test_opconnect_store_mints_and_persists_when_absent():
    state = {"fields": []}
