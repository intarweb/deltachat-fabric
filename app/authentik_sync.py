...
The fixed set_password (NOT the auto-minted app-token) is the LDAP bind credential — the mail
server binds ``cn=<bot>,ou=users,<base>`` with that password. Config from env (injected at
deploy); the Authentik URL + token are READ FROM op-connect (never plaintext env):
